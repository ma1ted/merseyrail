...
public class LiveMap : BaseFragment, GoogleMap.IOnInfoWindowClickListener, IJavaObject, IDisposable, IOnMapReadyCallback
{
	...
	private string passedStationName;

	private View _view;

	private bool actionsBound;

	private Marker usersLocationMarker;
	...
	private ImageButton directionsDrive { get; set; }

	private ImageButton directionsWalk { get; set; }

	private ImageButton backButton { get; set; }

	private string directionsTitle { get; set; }

	private LatLng directionsLatLng { get; set; }

	private Bundle _mapBundle { get; set; }
	...
	public LiveMap()
	{
		...
		stationService = SharedServices.IoC.Resolve<StationService>();
	...
	public override void OnCreate(Bundle savedInstanceState)
	{
		...
		{
			passedStationName = base.Arguments.GetString("passedStationName") ?? string.Empty;
		}
		if (SharedValues.CurrentDeviceLocation != null)
		{
			CurrentLocation = SharedValues.CurrentDeviceLocation;
			return;
		}
	...
	public override void OnResume()
	{
		...
		{
			locationService.OnLocationUpdated += plotUsersCurrentLocation;
		}
	}
	...
	private void plotUsersCurrentLocation(object sender, Location e)
	{
		if (mapFragment != null)
		{
			MarkerOptions markerOptions = new MarkerOptions();
			markerOptions.SetTitle("Your Location");
			markerOptions.SetSnippet(string.Empty);
			markerOptions.SetPosition(new LatLng(e.Latitude, e.Longitude));
			markerOptions.InvokeIcon(BitmapDescriptorFactory.FromResource(2131165401));
			if (usersLocationMarker != null)
			{
				usersLocationMarker.Remove();
			}
			usersLocationMarker = _map.AddMarker(markerOptions);
		}
	}
	...
	public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
	{
		...
		{
			InitMapFragment();
			_view = inflater.Inflate(2131361852, null, attachToRoot: false);
			directionsDrive = _view.FindViewById<ImageButton>(2131230811);
			directionsDrive.Alpha = 0.1f;
			directionsWalk = _view.FindViewById<ImageButton>(2131230812);
			directionsWalk.Alpha = 0.1f;
			backButton = _view.FindViewById<ImageButton>(2131230732);
			if (!actionsBound)
			{
				...
				directionsDrive.Click += delegate
				{
					if (directionsDrive.Alpha == 1f && SharedValues.CurrentDeviceLocation != null)
					{
						string uriString2 = $"https://www.google.com/maps/dir/{directionsLatLng.Latitude},{directionsLatLng.Longitude}/{SharedValues.CurrentDeviceLocation.Latitude},{SharedValues.CurrentDeviceLocation.Longitude}/@52.9944771,-2.6576473,6z/am=t/data=!3m1!4b1!4m4!4m3!1m0!1m0!3e0";
						Intent intent2 = new Intent("android.intent.action.VIEW", Android.Net.Uri.Parse(uriString2));
						StartActivity(intent2);
					}
				};
				directionsWalk.Click += delegate
				{
					if (directionsWalk.Alpha == 1f && SharedValues.CurrentDeviceLocation != null)
					{
						string uriString = $"https://www.google.com/maps/dir/{directionsLatLng.Latitude},{directionsLatLng.Longitude}/{SharedValues.CurrentDeviceLocation.Latitude},{SharedValues.CurrentDeviceLocation.Longitude}/@52.9944771,-2.6576473,6z/am=t/data=!3m1!4b1!4m4!4m3!1m0!1m0!3e2";
						Intent intent = new Intent("android.intent.action.VIEW", Android.Net.Uri.Parse(uriString));
	...
	{
		mapFragment = base.ChildFragmentManager.FindFragmentByTag("map_liveMap") as SupportMapFragment;
		if (mapFragment == null)
		{
			GoogleMapOptions options = new GoogleMapOptions().InvokeMapType(1).InvokeZoomControlsEnabled(enabled: false);
			Android.Support.V4.App.FragmentTransaction fragmentTransaction = base.ChildFragmentManager.BeginTransaction();
			mapFragment = SupportMapFragment.NewInstance(options);
			mapFragment.RetainInstance = true;
			mapFragment.GetMapAsync(this);
			fragmentTransaction.Replace(2131230730, mapFragment, "map_liveMap");
	...
	{
		if (marker.Title.ToLower().Contains("closures"))
		{
			((Main)base.Activity).CurrentFragment = this;
			((Main)base.Activity).ShowPlatformUpgrades();
		}
		else if (!marker.Title.ToLower().Contains("your"))
		{
			_ = base.Arguments;
			((Main)base.Activity).CurrentFragment = this;
			((Main)base.Activity).ShowDepartureStation(marker.Title);
		}
	}
	...
	private void PlotMarkers()
	{
		...
		foreach (Station station in stationService.GetStations(merseyrailOnly: true))
		{
			MarkerOptions markerOptions = new MarkerOptions();
			markerOptions.SetTitle(station.Name);
			markerOptions.SetSnippet(string.Empty);
			markerOptions.SetPosition(new LatLng(station.Lat, station.Lon));
			markerOptions.InvokeIcon(BitmapDescriptorFactory.FromResource(2131165394));
			Marker item = _map.AddMarker(markerOptions);
			markersList.Add(item);
		}
		moveToSelectedMarker();
	}

	private void moveToSelectedMarker()
	{
		if (!string.IsNullOrEmpty(passedStationName))
		{
			Marker marker = markersList.Where((Marker x) => x.Title == passedStationName).SingleOrDefault();
			if (marker != null)
			{
				marker.ShowInfoWindow();
				_map.MoveCamera(CameraUpdateFactory.NewLatLng(marker.Position));
				directionsLatLng = marker.Position;
				directionsDrive.Alpha = 1f;
				directionsWalk.Alpha = 1f;
			}
	...
	private void UpdateTrainMarker()
	{
		if (TrainMarker != null)
		{
			base.Activity.RunOnUiThread(delegate
			{
				...
				CameraUpdate update = CameraUpdateFactory.NewLatLngZoom(TrainPosition, 15f);
				_map.MoveCamera(update);
			});
		}
	}

	public void AddTrainMarker()
	{
		TrainMarkerOptions = new MarkerOptions();
		TrainMarkerOptions.SetTitle("Train Label");
		TrainMarkerOptions.SetSnippet("00:00");
		TrainMarkerOptions.SetPosition(new LatLng(CurrentLocation.Latitude, CurrentLocation.Longitude));
	...
	private List<HelpDisplayItem> GetHelpItemList()
	{
		...
		{
			HelpDisplay.NewHelpItem("btn_directions_drive", directionsDrive, HelpDisplay.HelpItemLayoutHint.Center, "Tap for driving directions to this station"),
			HelpDisplay.NewHelpItem("btn_directions_walk", directionsWalk, HelpDisplay.HelpItemLayoutHint.Center, "Tap for walking directions also")
		};
	}
