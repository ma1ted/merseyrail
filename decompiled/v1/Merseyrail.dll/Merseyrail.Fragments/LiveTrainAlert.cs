...
public class LiveTrainAlert : BaseFragment, IOnMapReadyCallback, IJavaObject, IDisposable
{
	...
	public LinearLayout CancelAlertButton { get; set; }

	public TextView Header { get; set; }

	public TextView FromNearestStation { get; set; }

	public TextView ViaStation { get; set; }

	public TextView FromTime { get; set; }

	public TextView TimeNow { get; set; }

	public TextView WalkingTime { get; set; }

	public TextView ALittleExtra { get; set; }

	public TextView CountDown { get; set; }

	public TextView CountdownMessage { get; set; }

	public TextView DestinationName { get; set; }

	public TextView LastUpdated { get; set; }

	public TimeSpan TimespanUntilTrain { get; set; }
	...
	private TrainReminderService trainReminderService { get; set; }

	private DepartureBoardService departureBoardService { get; set; }

	private LiveMapService liveMapService { get; set; }

	private bool AlertSet { get; set; }

	private Button ToggleAlertButton { get; set; }

	private LatLng TrainPosition { get; set; }

	private bool ActionsSet { get; set; }

	private ReminderInfo trainReminder { get; set; }

	public LiveTrainAlert()
	{
		trainReminderService = SharedServices.IoC.Resolve<TrainReminderService>();
	...
	public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
	{
		if (_view == null)
		{
			_view = inflater.Inflate(2131361854, null, attachToRoot: false);
		}
		BackButton = (ImageButton)_view.FindViewById(2131231003);
		Header = (TextView)_view.FindViewById(2131231000);
		DestinationName = (TextView)_view.FindViewById(2131231243);
		LastUpdated = (TextView)_view.FindViewById(2131231237);
		FromNearestStation = (TextView)_view.FindViewById(2131231235);
		ViaStation = (TextView)_view.FindViewById(2131231234);
		CountdownMessage = (TextView)_view.FindViewById(2131231233);
		FromTime = (TextView)_view.FindViewById(2131231236);
		TimeNow = (TextView)_view.FindViewById(2131231239);
		WalkingTime = (TextView)_view.FindViewById(2131231240);
		ALittleExtra = (TextView)_view.FindViewById(2131231238);
		CountDown = (TextView)_view.FindViewById(2131231232);
		ToggleAlertButton = (Button)_view.FindViewById(2131230815);
	...
	private void SetActions()
	{
		...
		ToggleAlertButton.Click += delegate
		{
			trainReminderService.SetReminderStatus(SharedSettings.FirebaseToken, DeviceType.Android, TrainId, trainReminder == null || !trainReminder.IsActive, delegate(bool success)
			{
				if (success)
				{
					trainReminder = trainReminderService.GetActiveReminder();
					AlertSet = trainReminder != null && trainReminder.IsActive;
					SetButtonState(AlertSet);
				}
				else
				{
					base.Activity.RunOnUiThread(delegate
					{
						Toast.MakeText(base.Activity, "Could not set the alert. Please try again.", ToastLength.Short)!.Show();
					});
				}
			});
		};
	...
	{
		bool buttonState = trainReminder != null && trainReminder.IsActive;
		SetButtonState(buttonState);
	}

	private void SetButtonState(bool state)
	{
		if (trainReminder != null)
		{
			PopulateForm(trainReminder);
		}
		if (state)
		{
			base.Activity.RunOnUiThread(delegate
			{
				ToggleAlertButton.Text = "Cancel Reminder";
			});
			return;
		}
		base.Activity.RunOnUiThread(delegate
		{
			ToggleAlertButton.Text = "Set Reminder";
		});
		StopCountdown();
		HideCountdownArea();
	}
	...
	public void InitTrainMarker()
	{
		...
		base.Activity.RunOnUiThread(delegate
		{
			if (TrainMarker == null)
			{
				...
				CameraUpdate update = CameraUpdateFactory.NewLatLngZoom(TrainPosition, 16f);
	...
	private void UpdateTrainMarker()
	{
		if (TrainMarker != null)
		{
			base.Activity.RunOnUiThread(delegate
			{
				...
				CameraUpdate update = CameraUpdateFactory.NewLatLngZoom(TrainPosition, _map.CameraPosition.Zoom);
				_map.MoveCamera(update);
			});
		}
	}
	...
	public void UpdateViewContent(TrainProgress progress)
	{
		...
		base.Activity.RunOnUiThread(delegate
		{
			...
			if (Header != null)
			{
				Header.Text = "Train Alert";
			}
			if (DestinationName != null)
			{
				DestinationName.Text = progress.DestinationStation.Name;
			}
			Station headingStation = progress[0].HeadingStation;
			if (headingStation.CrsCode != progress.DestinationStation.CrsCode)
			{
				ViaStation.Text = "via " + headingStation.Name;
			}
			else
			{
				ViaStation.Text = "";
	...
	public override void OnResume()
	{
		...
		{
			trainReminder = trainReminderService.GetActiveReminder();
			TrainId = trainReminder.TrainId;
		}
		else
		{
			SetTrainReminder(TrainId);
		}
		InitMapFragment();
		SetButtonState();
	}

	public void SetTrainReminder(string trainid)
	{
		if (SharedValues.CurrentDeviceLocation != null)
		{
			trainReminderService.SetTrainReminder(SharedSettings.FirebaseToken, DeviceType.Android, trainid, SharedValues.CurrentDeviceLocation.Latitude, SharedValues.CurrentDeviceLocation.Longitude, delegate(ReminderInfo ri)
			{
				AlertSet = ri.IsActive;
				PopulateForm(ri);
				UpdateViewContent(ri.TrainProgress);
			}, delegate
			{
				AlertSet = false;
			});
	...
	private void PopulateForm(ReminderInfo reminderInfo)
	{
		if (reminderInfo != null && base.Activity != null)
		{
			base.Activity.RunOnUiThread(delegate
			{
				_ = reminderInfo.TrainProgress.Where((TrainStop x) => x != null).Last().Station;
				DestinationName.Text = reminderInfo.TrainProgress.DestinationStation.Name;
				Station headingStation = reminderInfo.TrainProgress.First().HeadingStation;
				if (headingStation.CrsCode != reminderInfo.TrainProgress.DestinationStation.CrsCode)
				{
					ViaStation.Text = "via " + headingStation.Name;
				}
				else
				...
				FromNearestStation.Text = reminderInfo.NearestStation.Name;
				FromTime.Text = reminderInfo.TrainDeparture;
				TimeNow.Text = DateTime.Now.ToShortTimeString();
				int num = reminderInfo.WalkingTime + reminderInfo.ContingencyTime;
				WalkingTime.Text = num / 60 + " mins";
				if (reminderInfo.IsActive)
				{
					InitCountdown(reminderInfo);
					UpdateCountdown(reminderInfo);
				}
			});
		}
	...
	private void HideCountdownArea()
	{
		base.Activity.RunOnUiThread(delegate
		{
			((LinearLayout)_view.FindViewById(2131231231)).Visibility = ViewStates.Gone;
		});
	}

	private void InitCountdown(ReminderInfo reminderInfo)
	{
		StopCountdown();
	...
	private void UpdateCountdown(ReminderInfo reminderInfo)
	{
		LinearLayout linearLayout = (LinearLayout)_view.FindViewById(2131231231);
		linearLayout.Visibility = ViewStates.Visible;
		if (reminderInfo.LocationDeparture.HasValue && reminderInfo.LocationDeparture.Value > DateTime.MinValue)
		{
			TimespanUntilTrain = reminderInfo.LocationDeparture.Value.Subtract(DateTime.Now);
			CountDown.Text = $"{Math.Abs((int)TimespanUntilTrain.TotalMinutes):00}:{Math.Abs(TimespanUntilTrain.Seconds):00}";
			if (TimespanUntilTrain.TotalSeconds < 0.0)
			{
				linearLayout.SetBackgroundColor(Color.Argb(255, 227, 7, 7));
				CountdownMessage.SetTextColor(Color.Argb(255, 255, 255, 255));
				CountdownMessage.Text = "You should have already left!";
			}
			else
			{
				CountdownMessage.Text = "You must leave your current location in:";
				CountdownMessage.SetTextColor(Color.Argb(255, 0, 212, 0));
		...
		{
			CountDown.Text = "";
			CountdownMessage.Text = "You're too late to catch that train!";
			CountdownMessage.SetTextColor(Color.Argb(255, 0, 0, 0));
			linearLayout.SetBackgroundColor(Color.Argb(255, 227, 7, 7));
		}
	}
	...
	{
		_mapFragment = base.ChildFragmentManager.FindFragmentByTag("map_livetrain") as SupportMapFragment;
		if (_mapFragment == null)
		{
			...
			_mapFragment.RetainInstance = true;
			_mapFragment.GetMapAsync(this);
			fragmentTransaction.Add(2131230998, _mapFragment, "map_livetrain");
	...
	{
		if (_mapFragment == null || _map == null)
		{
			return;
		}
		AddPolylines();
